...
a = LOAD '$loadList' USING org.commoncrawl.pig.ArcLoader()  AS (charset: chararray, length, type: chararray, statuscode, ipaddress, url: chararray, html);
...
#print $pigString;
   ...
   print HANDLE "$pigString";
    close (HANDLE);
   system("pig -f tmpfile.pig");
    system("rm tmpfile.pig");
}
